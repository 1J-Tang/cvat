 ...
 * @param target
 */
export function patchShortcutsForLocaleOn(target: Record<string, KeyMapItem>): OffFunction {
    const applyLocale = (lng: string, i18next: I18next): void => {
        ...
        Object.entries(info).forEach(([shortcutID, translation]) => {
            if (shortcutID in target && Array.isArray(translation)) {
                const shortcut = target[shortcutID];
                [shortcut.name, shortcut.description] = translation;
            }
        });
